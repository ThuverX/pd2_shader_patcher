...
bool ApplyShaderPatch(ShaderPackage* package_ptr, ShaderPatch* patch_ptr)
{
	...
	// TODO: keep and eye on this, i've already verified that it works, but just make sure to check this first if it
	// doesn't work anymore
	int64_t namehash = pd2_create_hash(patch_ptr->name.c_str());
	...
	if (patch_ptr->mode == SHADER_PATCH_MODE_REPLACE)
	{
		...
		for (auto it = patch_ptr->passes.begin(); it != patch_ptr->passes.end(); ++it)
		{
			...
			int64_t pass_namehash = pd2_create_hash(pass_name.c_str());
